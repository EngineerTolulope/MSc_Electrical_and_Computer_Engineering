...
% with 0.5 sec square window epochs, with no overlap. It superimposes the
% results of the In-house averaging and PWelch averaging method.
%
...
% In-house averaging
index=1;
for i=1:Nseg:length(emg) %increments i by Nseg
    %Calculates the power spectrum
    Pow(:,index) = 2*(abs(fft(emg(i:(i+Nseg-1))))/N).^2;
    
    index = index+1;
end
EMG = mean(Pow,2);  %Gets the mean by columns
...
hold on
plot(f(f<fs/2), EMG(f<fs/2), 'k-.')
xlabel('Frequency in Hertz');
ylabel('Power Spectral Density in V^2/Hz');
title('Power spectrum of the EMG signal with in-house averaging of 0.5 epochs');
...
Pfd1 = sum(EMG(f<fs/2)); %power of the signal in the frequency domain
disp([Ptd Pfd Pfd1])
