...
%% Question 4 Rework to include M-file
% clear;clc;close all;
% % Starting values
% x = 0.7; %Starting value
% f = 8;   %No of terms
% true_val = sin(x);
% 
% % Computation
% for N = 1:1:f
%    appr_val = Q4_sin(x,N);
%    Percent_Error = (true_val-appr_val)/true_val * 100;
%    fprintf('Sin (%0.1f) = %0.16f with percent relative error= %0.16f%% \n \n',x,f,Percent_Error);
% end

% function [ value ] = Q4_sin( x, N_term)
% % The m-file to manually solve for sin(x)
% value = 0;
% for c = 1:1:N_term
%     value = value + (-1)^(c+1)* x^(2*c -1)/factorial(2*c-1);
% end
% end
